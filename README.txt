...
folder_projektu/
│
├── train_lstm_CORRECTED.py     ← skrypt trenujący model
...
## 1. Skrypt trenujący – `train_lstm_CORRECTED.py`
...
```powershell
python train_lstm_CORRECTED.py
```

**Trenuj tylko na konkretnych słowach:**
```powershell
python train_lstm_CORRECTED.py --classes "apple,dog,fish,cry,dance"
```

**Trenuj na pierwszych N słowach (alfabetycznie):**
```powershell
python train_lstm_CORRECTED.py --num-classes 10
```

**Zmień liczbę epok i batch size:**
```powershell
python train_lstm_CORRECTED.py --epochs 50 --batch 64
```

**Trenuj z konkretnych klas i ustaw własny folder z danymi:**
```powershell
python train_lstm_CORRECTED.py --data "C:\moje_dane" --classes "pies,kot,ryba"
```

**Wymuś przeładowanie danych (ignoruj cache):**
```powershell
python train_lstm_CORRECTED.py --refresh-cache
```

**Wyłącz cache całkowicie:**
```powershell
python train_lstm_CORRECTED.py --no-cache
```
...
2. Wytrenuj model → python train_lstm_CORRECTED.py --classes "znak1,znak2,znak3"
