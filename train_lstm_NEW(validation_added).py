...
# 1.05.26 ZMIANA KODU accuracy przed ~68%, accuracy po zmianie ~
...
def train(data_path: str, epochs: int, batch_size: int):
    """
    Trenuje model LSTM z użyciem:
      1. osobnego zbioru testowego,
      2. walidacji K-Fold na danych treningowych,
      3. finalnego treningu modelu,
      4. końcowej ewaluacji na zbiorze testowym.
    """
    ...
    # 2. Oddzielenie zbioru testowego
    # Test set NIE bierze udziału w K-Fold.
    # Służy tylko do końcowej oceny modelu.
    ...
    # 3. Walidacja K-Fold na zbiorze treningowym
    # Dane treningowe są dzielone na 5 grup.
    # W każdej iteracji inna grupa pełni rolę walidacji.
    n_splits = 5
    skf = StratifiedKFold(
        n_splits=n_splits,
        shuffle=True,
    ...
    for fold, (train_idx, val_idx) in enumerate(
        ...
        # Podział danych w aktualnym foldzie
        ...
        # Dla każdego folda budujemy NOWY model
        # To ważne, bo każdy fold musi zaczynać uczenie od zera.
        model = build_lstm_model(num_classes)
        ...
        # Trening modelu na aktualnym foldzie
        history = model.fit(
            X_train,
            y_train,
    ...
    # 4. Podsumowanie walidacji K-Fold
    print(f"\n{'═' * 50}")
    ...
    # 5. Finalny trening modelu
    # Po sprawdzeniu jakości na K-Fold trenujemy finalny model
    # na całym zbiorze train+validation.
    print(f"\n{'═' * 50}")
    ...
    # Uwaga:
    # Tutaj X_test jest używany jako validation_data tylko po to,
    # żeby Keras mógł monitorować val_loss i val_accuracy podczas finalnego treningu.
    ...
    # 6. Końcowa ewaluacja na zbiorze testowym
